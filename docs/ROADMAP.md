...
- 📋 **S1-2. StorageService 추상화 구현** [복잡도: M]
  - `StorageService` 인터페이스 정의: `upload(gameId, fileName, bytes)`, `getUrl(gameId, fileName)`, `delete(gameId, fileName)`, `listFiles(gameId)`
...
### Sprint 2: 기존 크롤러 마이그레이션 + 클라이언트 연결 (2주)
...
2. curl POST로 크롤링 트리거 또는 스케줄러 대기
...
### Sprint 3: 관리자 UI 프론트엔드 + AI 크롤러 기초 (2주)
...
### Sprint 5: 자동 태그 + AI 추천 + 검색 API (2주)
...
| 마일스톤 | 예상 시점 | 핵심 산출물 |
|----------|-----------|-------------|
| **M1: 인프라 기초 완료** | Sprint 2 종료 (4주차) | Docker 환경에서 6개 게임 크롤링 동작, Flutter 앱 API 연결 |
| **M2: AI 크롤러 MVP** | Sprint 4 종료 (8주차) | URL 등록만으로 AI 파싱 전략 생성 + 크롤링, 관리자 UI |
| **M3: Phase 2 완료** | Sprint 5 종료 (10주차) | 태그 자동 생성, 검색, AI 추천 기능 |
| **M4: 안정화 완료** | Sprint 6 종료 (12주차) | 클라이언트 리팩토링, 테스트 커버리지 확보 |
| **M5: v1.0 릴리스** | Sprint 8 종료 (16주차) | 데스크탑 지원, 좋아요, AI 설명, API 문서 |
